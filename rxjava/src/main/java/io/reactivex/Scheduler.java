...
public abstract class Scheduler {
    ...
    public Disposable schedulePeriodicallyDirect(Runnable run, long initialDelay, long period, TimeUnit unit) {
        final ArrayCompositeResource<Disposable> acr = new ArrayCompositeResource<>(2, Disposables.consumeAndDispose());
        final Worker w = createWorker();
        acr.lazySet(0, w);
    ...
    public static abstract class Worker implements Disposable {
        ...
        public Disposable schedulePeriodically(Runnable run, final long initialDelay, final long period, final TimeUnit unit) {
            final MultipleAssignmentResource<Disposable> first = new MultipleAssignmentResource<>(Disposables.consumeAndDispose());
