...
public final class OperatorTakeUntil<T, U> implements Operator<T, T> {
    ...
    public Subscriber<? super T> apply(Subscriber<? super T> child) {
        final SerializedSubscriber<T> serial = new SerializedSubscriber<>(child);
        
        final ArrayCompositeResource<Subscription> frc = new ArrayCompositeResource<>(2, SubscriptionHelper.consumeAndCancel());
        
        final TakeUntilSubscriber<T> tus = new TakeUntilSubscriber<>(serial, frc);
