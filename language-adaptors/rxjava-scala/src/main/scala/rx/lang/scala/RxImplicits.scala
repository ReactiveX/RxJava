...
object RxImplicits {
    ...
    implicit class ScalaObservable[A](wrapped: Observable[A]) {
        ...
        class WithFilter(p: A => Boolean) {
            ...
            def foreach(f: A => Unit): Unit = wrapped.filter(p).forEach(f)
...
class UnitTestSuite extends JUnitSuite {
    ...
    @Test def testSingle {
        assertEquals(1, Observable.from(1).single)
    }
    
    @Test def testSinglePredicate {
        val found = Observable.from(1, 2, 3).single(isEven)
        assertEquals(2, found)
    }
    
    @Test def testSingleOrDefault {
        assertEquals(0, Observable.from().singleOrDefault(0))
        assertEquals(1, Observable.from(1).singleOrDefault(0))
        try {
            Observable.from(1, 2, 3).singleOrDefault(0)
    ...
    @Test def testSingleOrDefaultPredicate {
        assertEquals(2, Observable.from(1, 2, 3).singleOrDefault(0, isEven))
        assertEquals(0, Observable.from(1, 3).singleOrDefault(0, isEven))
        try {
            Observable.from(1, 2, 3).singleOrDefault(0, isOdd)
            fail("Did not catch any exception, expected IllegalStateException")
        } catch {
            case ex: IllegalStateException => println("Caught expected IllegalStateException")
            case ex: Throwable => fail("Caught unexpected exception " + ex.getCause + ", expected IllegalStateException")
    ...
    @Test def testLast {
        ...
        assertEquals(4, observable.last)
    }
    
    @Test def testLastPredicate {
        val observable = Observable.from(1, 2, 3, 4)
        assertEquals(3, observable.last(isOdd))
    }
    
    @Test def testLastOrDefault {
        val observable = Observable.from(1, 2, 3, 4)
        assertEquals(4, observable.lastOrDefault(5))
        assertEquals(5, Observable.from[Int]().lastOrDefault(5))
    }
    
    @Test def testLastOrDefaultPredicate {
        val observable = Observable.from(1, 2, 3, 4)
        assertEquals(3, observable.lastOrDefault(5, isOdd))
        assertEquals(5, Observable.from[Int]().lastOrDefault(5, isOdd))
    }
    ...
    @Test def testOnErrorResumeNextFuncNoError {
        val observable = Observable.from(1, 2, 3, 4)
        val resumeFunc = (ex: RuntimeException) => Observable.from(5, 6, 7, 8)
    ...
    @Test def testOnErrorResumeNextFuncErrorOccurs {
        val observable = new ObservableWithException(subscription, "foo", "bar")
        val resumeFunc = (ex: RuntimeException) => Observable.from("a", "b", "c", "d")
        val observableWithErrorHandler = observable.onErrorResumeNext(resumeFunc)
        observableWithErrorHandler.subscribe(observer.asInstanceOf[Observer[String]])
    ...
    @Test def testOnErrorReturnFuncNoError {
        val observable = Observable.from(1, 2, 3, 4)
        val returnFunc = (ex: RuntimeException) => 87
    ...
    @Test def testOnErrorReturnFuncErrorOccurs {
        val observable = new ObservableWithException(subscription, "foo", "bar")
        val returnFunc = (ex: RuntimeException) => "baz"
        val observableWithErrorHandler = observable.onErrorReturn(returnFunc)
        observableWithErrorHandler.subscribe(observer.asInstanceOf[Observer[String]])
    ...
    @Test def testReduce {
        val observable = Observable.from(1, 2, 3, 4)
        assertEquals(10, observable.reduce((a: Int, b: Int) => a + b).single)
    }
    
    @Test def testReduceWithInitialValue {
        val observable = Observable.from(1, 2, 3, 4)
        assertEquals(110, observable.reduce(100, (a: Int, b: Int) => a + b).single)
    }
    ...
    @Test def testToIterable {
        ...
        val it = observable.toIterable.iterator
