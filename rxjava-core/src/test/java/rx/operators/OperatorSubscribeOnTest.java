...
public class OperatorSubscribeOnTest {
    ...
    public static class SlowScheduler extends Scheduler {
        ...
        public EventLoop createEventLoop() {
            return new SlowInnerScheduler(actual.createEventLoop());
        }

        private class SlowInnerScheduler extends Scheduler.EventLoop {

            private final EventLoop actualInner;

            public SlowInnerScheduler(EventLoop actualInner) {
            ...
            @Override
            public void schedule(final Action1<Scheduler.Schedulable> action) {
                actualInner.schedule(action, delay, unit);
            }

            @Override
            public void schedule(final Action1<Scheduler.Schedulable> action, final long delayTime, final TimeUnit delayUnit) {
