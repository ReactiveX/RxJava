...
public class Functions {
    ...
    public static FuncN fromAction(Action action) {
        ...
        if (action instanceof Action0) {
            ...
            return fromAction((Action1) action);
        } else if (action instanceof Action2) {
            return fromAction((Action2) action);
        } else if (action instanceof Action3) {
            return fromAction((Action3) action);
        }

        throw new RuntimeException("Unknown implementation of Action: " + action.getClass().getSimpleName());
    }
