...
public class TestScheduler extends Scheduler {
    ...
    private static class TimedAction {
        ...
        private TimedAction(EventLoop scheduler, long time, Action1<Schedulable> action) {
            this.time = time;
            this.action = action;
    ...
    private void triggerActions(long targetTimeInNanos) {
        while (!queue.isEmpty()) {
            ...
            if (!current.scheduler.isUnsubscribed()) {
                current.action.call(Schedulable.create(current.scheduler, current.action));
            }
        }
    ...
    private final class InnerTestScheduler extends EventLoop {
