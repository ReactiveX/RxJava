...
public final class OperatorWindowWithTime<T> implements Operator<Observable<T>, T> {
    ...
    static final class State<T> {
        ...
        final int count;
        static final State<Object> EMPTY = new State<Object>(null, 0);
        
        public State(Subject<T, T> subject, int count) {
            this.subject = subject;
            this.count = count;
        }
        public State<T> next() {
            return new State<T>(subject, count + 1);
        }
        public State<T> create(Subject<T, T> s) {
            return new State<T>(s, 0);
        }
    ...
    final class ExactSubscriber extends Subscriber<T> {
        ...
        void replaceSubject() {
            Subject<T, T> s = state.subject;
            if (s != null) {
                s.onCompleted();
            }
            s = createSubject();
            state = state.create(s);
            child.onNext(s);
        }
        void emitValue(T t) {
            State<T> s = state;
    ...
    static final class CountedSerializedSubject<T> {
        final Subject<T, T> subject;
        final Observer<T> serialized;
        int count;

        public CountedSerializedSubject(Subject<T, T> subject) {
            this.subject = subject;
            this.serialized = new SerializedObserver<T>(subject);
        }
    }
    ...
    final class InexactSubscriber extends Subscriber<T> {
        ...
        public void onNext(T t) {
            ...
            for (CountedSerializedSubject<T> cs : list) {
                cs.serialized.onNext(t);
                if (cs.count == size) {
                    cs.serialized.onCompleted();
                }
            }
        }
        ...
        void terminateChunk(CountedSerializedSubject<T> chunk) {
            ...
            if (terminate) {
                chunk.serialized.onCompleted();
            }
        }
        CountedSerializedSubject<T> createCountedSerializedSubject() {
            return new CountedSerializedSubject<T>(PublishSubject.<T>create());
        }
    }
}
