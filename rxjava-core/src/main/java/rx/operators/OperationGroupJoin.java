...
public class OperationGroupJoin<T1, T2, D1, D2, R> implements OnSubscribeFunc<R> {
    ...
    class ResultManager implements Subscription {
        ...
        public void init() {
            ...
            s1.set(left.unsafeSubscribe(new LeftObserver(s1)));
            s2.set(right.unsafeSubscribe(new RightObserver(s2)));
        ...
        class LeftObserver extends Subscriber<T1> {
            ...
            public void onNext(T1 args) {
                try {
                    ...
                    group.add(sduration);
                    sduration.set(duration.unsafeSubscribe(new LeftDurationObserver(id, sduration, subj)));
        ...
        class RightObserver extends Subscriber<T2> {
            ...
            public void onNext(T2 args) {
                try {
                    ...
                    SerialSubscription sduration = new SerialSubscription();
                    group.add(sduration);
                    sduration.set(duration.unsafeSubscribe(new RightDurationObserver(id, sduration)));
    ...
    static class WindowObservableFunc<T> implements OnSubscribeFunc<T> {
        ...
        public Subscription onSubscribe(Observer<? super T> t1) {
            ...
            cs.add(underlying.unsafeSubscribe(wo));
            return cs;
        }
