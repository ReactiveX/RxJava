...
public class FlowableMapTest extends RxJavaTest {
    ...
    public void mapMany2() {
        ...
        Flowable<String> m = f.flatMap((Function<Flowable<Map<String, String>>, Flowable<String>>) f1 -> f1.map(map -> map.get("firstName")));
        m.subscribe(stringSubscriber);
    ...
    public void functionCrashUnsubscribes() {
        ...
        pp.map((Function<Integer, Integer>) v -> {
    ...
    public void sourceIgnoresCancel() {
        ...
        try {
            Flowable.fromPublisher((Publisher<Integer>) s -> {
    ...
    public void badSource() {
        TestHelper.checkBadSourceFlowable((Function<Flowable<Object>, Object>) f -> f.map(Functions.identity()), false, 1, 1, 1);
    }
