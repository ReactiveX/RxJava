...
public class FlowableReduceTest extends RxJavaTest {
    ...
    public void doubleOnSubscribe() {
        TestHelper.checkDoubleOnSubscribeFlowableToMaybe((Function<Flowable<Integer>, MaybeSource<Integer>>) f -> f.reduce(sum));
    }
    ...
    public void shouldReduceTo10Events() {
        ...
        Flowable.range(0, 10).flatMap((Function<Integer, Publisher<String>>) x -> Flowable.range(0, 2)
            .map(y -> blockingOp(x, y)).subscribeOn(Schedulers.io())
        .reduce((l, r) -> l + "_" + r)
        .doOnSuccess(s -> {
    ...
    public void seedDoubleOnSubscribe() {
        TestHelper.checkDoubleOnSubscribeFlowableToSingle((Function<Flowable<Integer>, SingleSource<Integer>>) f -> f.reduce(0, (a, b) -> a));
    }

    @Test
    public void seedDisposed() {
        TestHelper.checkDisposed(PublishProcessor.<Integer>create().reduce(0, (a, b) -> a));
    }
