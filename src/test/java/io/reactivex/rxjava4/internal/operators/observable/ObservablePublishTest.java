...
public class ObservablePublishTest extends RxJavaTest {
    ...
    public void takeUntilWithPublishedStreamUsingSelector() {
        ...
        Observable<Integer> xs = Observable.range(0, Flowable.bufferSize() * 2).doOnNext(_ -> emitted.incrementAndGet());
        TestObserver<Integer> to = new TestObserver<>();
    ...
    public void selectorDisconnectsIndependentSource() {
        ...
        ps.publish(_ -> Observable.range(1, 2))
        .test()
        .assertResult(1, 2);
    ...
    public void selectorInnerError() {
        ...
        ps.publish((Function<Observable<Integer>, ObservableSource<Integer>>) _ -> Observable.error(new TestException()))
        .test()
        .assertFailure(TestException.class);
    ...
    public void doubleOnSubscribe() {
        TestHelper.checkDoubleOnSubscribeObservable(o -> Observable.<Integer>never().publish(_ -> o)
        );
    }
