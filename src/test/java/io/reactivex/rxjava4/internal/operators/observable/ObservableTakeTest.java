...
public class ObservableTakeTest extends RxJavaTest {
    ...
    public void takeWithErrorHappeningInTheLastOnNext() {
        Observable<Integer> w = Observable.fromIterable(Arrays.asList(1, 2, 3)).take(1).map(_ -> {
            throw new IllegalArgumentException("some error");
        });
    ...
    private static Observable<Long> INFINITE_OBSERVABLE = Observable.unsafeCreate(op -> {
    ...
    public void reentrantTake() {
        ...
        source.take(1).doOnNext(_ -> source.onNext(2)).subscribe(to);
