...
public class ObservableGroupByTest extends RxJavaTest {
        ...
        SYNC_INFINITE_OBSERVABLE_OF_EVENT(4, subscribeCounter, sentEventCounter)
                ...
                .flatMap((Function<GroupedObservable<Integer, Event>, Observable<String>>) eventGroupedObservable -> {
                    ...
                    if (eventGroupedObservable.getKey() >= 2) {
                        // filter these
                        eventStream = eventGroupedObservable.filter(_ -> false);
                    }
    ...
    public void groupByBackpressure3() throws InterruptedException {
        ...
        .doOnEach(t1 -> System.out.println("NEXT: " + t1)).subscribe(to);
        to.awaitDone(5, TimeUnit.SECONDS);
        to.assertNoErrors();
    }
    ...
    public void groupByBackpressure2() throws InterruptedException {
        ...
        Observable.range(1, 4000).groupBy(IS_EVEN2)
        .flatMap(g -> g.take(2).observeOn(Schedulers.computation()).map(l -> {
            if (g.getKey()) {
                try {
                    Thread.sleep(1);
                } catch (InterruptedException e) {
    ...
    public void groupByWithNullKey() {
        ...
        Observable.just("a", "b", "c").groupBy((Function<String, String>) _ -> null).subscribe(groupedObservable -> {
    ...
    public void groupByUnsubscribe() {
        ...
        o.groupBy((Function<Integer, Integer>) _ -> null).subscribe(to);
    ...
    public void delayErrorCompleteMoreWorkInGroup() {
        ...
        TestObserver<Integer> to = ps.groupBy(_ -> 1,new StandardBufferedConfig(true))
