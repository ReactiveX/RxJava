...
public class ObservableRetryWithPredicateTest extends RxJavaTest {
    ...
    public void retryTwiceAndGiveUp() {
        Observable<Integer> source = Observable.unsafeCreate(t1 -> {
            t1.onSubscribe(Disposable.empty());
            t1.onNext(0);
            t1.onNext(1);
            t1.onError(new TestException());
    ...
    public void unsubscribeFromRetry() {
        ...
        Disposable sub = subject.retry(retryTwice).subscribe(_ -> count.incrementAndGet());
        subject.onNext(1);
        sub.dispose();
        subject.onNext(2);
    ...
    public void issue2826() {
        ...
        Observable.just(1).map((Function<Integer, Integer>) _ -> {
            c.incrementAndGet();
            throw e;
    ...
    public void justAndRetry() throws Exception {
        ...
        int value = Observable.just(1).map(t1 -> {
    ...
    public void retryBiPredicateDisposeRace() {
        for (int i = 0; i < TestHelper.RACE_DEFAULT_LOOPS; i++) {
            ...
            final TestObserver<Integer> to = ps.retry((BiPredicate<Object, Object>) (_, _) -> true).test();
