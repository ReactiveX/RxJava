...
public enum TestHelper {
    ...
    public static void assertBadRequestReported(Publisher<?> source) {
        ...
        try {
            ...
            assertEquals(list.getFirst().getMessage(), "n > 0 required but it was -99");
        } finally {
    ...
    public static void checkDisposed(Disposable d) {
        ...
        assertTrue(d.isDisposed(), "Not disposed again?!");
    }
    ...
    public static <T> void checkDoubleOnSubscribeParallel(Function<ParallelFlowable<T>, ? extends ParallelFlowable<?>> transform) {
        ...
        try {
            ...
            assertEquals(false, b[0], "Rail 1 First disposed?");
            assertEquals(true, b[1], "Rail 1 Second not disposed?");

            assertEquals(false, b[2], "Rail 2 First disposed?");
            assertEquals(true, b[3], "Rail 2 Second not disposed?");
    ...
    public static <T, U> void checkDisposedMaybe(Function<Maybe<T>, ? extends MaybeSource<U>> composer) {
        ...
        assertTrue(pp.hasSubscribers(), "Not subscribed to source!");

        ts.cancel();

        assertFalse(pp.hasSubscribers(), "Dispose not propagated!");
    }
    ...
    public static File findSource(String baseClassName) throws Exception {
        return findSource(baseClassName, "io.reactivex.rxjava4.core");
    }
     ...
     * path and return a file to it or null if not found.
     * @param baseClassName the class name such as {@code Maybe}
     * @param parentPackage the parent package such as {@code io.reactivex.rxjava4.core}
     * @return the File pointing to the source
     * @throws Exception on error
     */
...
//        System.out.println(path);
    ...
    public static <T> void checkUndeliverableUponCancel(FlowableConverter<Integer, T> transform) {
        ...
        try {
            ...
            assertFalse(errors.isEmpty(), "No undeliverable errors received");
