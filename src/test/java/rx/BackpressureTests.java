...
public class BackpressureTests {
    ...
    private static Observable<Integer> incrementingIntegers(final AtomicInteger counter, final ConcurrentLinkedQueue<Thread> threadsSeen) {
        return Observable.create(new OnSubscribe<Integer>() {
            ...
            @Override
            public void call(final Subscriber<? super Integer> passS) {

                WeakReference<Subscriber<? super Integer>> s = new WeakReference<Subscriber<? super Integer>>(passS);
                passS.setProducer(new Producer() {
                    int i = 0;
