 ...
 */
public final class OperatorMergeSingle<T> implements Operator<T, Single<? extends T>> {
    ...
    static final class HolderNoDelay {
        ...
        static final OperatorMergeSingle<Object> INSTANCE = new OperatorMergeSingle<Object>(false, Integer.MAX_VALUE);
    }
    /** Lazy initialization via inner-class holder. */
    static final class HolderDelayErrors {
        /** A singleton instance. */
        static final OperatorMergeSingle<Object> INSTANCE = new OperatorMergeSingle<Object>(true, Integer.MAX_VALUE);
    }
    /**
    ...
    public static <T> OperatorMergeSingle<T> instance(boolean delayErrors) {
        if (delayErrors) {
            return (OperatorMergeSingle<T>)HolderDelayErrors.INSTANCE;
        }
        return (OperatorMergeSingle<T>)HolderNoDelay.INSTANCE;
    }
    /**
     ...
     */
    public static <T> OperatorMergeSingle<T> instance(boolean delayErrors, int maxConcurrent) {
        if (maxConcurrent <= 0) {
        ...
        return new OperatorMergeSingle<T>(delayErrors, maxConcurrent);
    }

    OperatorMergeSingle(boolean delayErrors, int maxConcurrent) {
        this.delayErrors = delayErrors;
        this.maxConcurrent = maxConcurrent;
    }

    @Override
    public Subscriber<Single<? extends T>> call(final Subscriber<? super T> child) {
     ...
     * @param <T> the value type
     */
    static final class MergeSubscriber<T> extends Subscriber<Single<? extends T>> {
        ...
        @Override
        public void onNext(Single<? extends T> t) {
            if (t == null) {
                return;
            }
            if (t instanceof ScalarSynchronousSingle) {
                tryEmit(((ScalarSynchronousSingle<? extends T>)t).get());
            } else {
