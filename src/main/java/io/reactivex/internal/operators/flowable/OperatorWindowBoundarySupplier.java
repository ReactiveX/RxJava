...
public final class OperatorWindowBoundarySupplier<T, B> implements Operator<Flowable<T>, T> {
    ...
    public Subscriber<? super T> apply(Subscriber<? super Flowable<T>> t) {
        return new WindowBoundaryMainSubscriber<T, B>(new SerializedSubscriber<Flowable<T>>(t), other, bufferSize);
    }
