...
public final class NbpOperatorObserveOn<T> implements NbpOperator<T, T> {
    ...
    static final class ObserveOnSubscriber<T> extends Padding0 implements NbpSubscriber<T>, Disposable, Runnable {
        /** */
        ...
        public ObserveOnSubscriber(NbpSubscriber<? super T> actual, Scheduler.Worker worker, boolean delayError, int bufferSize) {
            this.actual = actual;
            this.worker = worker;
            this.delayError = delayError;
            this.bufferSize = bufferSize;
        ...
        boolean checkTerminated(boolean d, boolean empty, NbpSubscriber<? super T> a) {
            if (cancelled) {
                s.dispose();
                worker.dispose();
                return true;
